...
> 本篇继续使用“数据库可证、源码可证、Skill 源码可证、Wiki/Trace 可证、能力推演”五级证据。Skill 证据来自 `/Users/yaao/Documents/code/tcum-ai-skills`，可以证明 SOP、脚本和 MCP 调用约束，但不能证明线上运行时正在使用 Git HEAD；没有生产 trace 的案例不会包装成线上实测。Knot 与 AICoop 是外部 Agent 平台，本文仍只分析 TCUM-AI 的协议适配。
...
Skill 仓还出现了新的 `grafana-zhiyan-dashboard-menu-transfer`，可迁移智研菜单中的自研 panel，也可复制 iframe Grafana panel 并按 `datasourceMappings` 替换 InfluxDB、Prometheus、MySQL、Elasticsearch、CLS 等数据源。它不在当前数据库快照列出的 9 个绑定 Skill 中，因此应标记为“仓库新增、运行时绑定待确认”，不能直接宣传成已上线入口。

### 8.1.1 生成类 Skill 到底怎么路由

| 用户场景 | Skill | 指标/查询来源 | Builder 与关键变量 |
|---|---|---|---|
| 明确“应用监控大盘”且给出 MStackCode、MStackEntityCode | `grafana-tcum-app` | `ListTenantEntities → FindMetrics → ListMetricLabels → PrometheusQuery` | `BuildTCUMDashboard(flavor=app)`；必须有 entity textbox 和关联信息 Table，PromQL 用 `=` |
| 未给 MStack、未指定数据源的内部监控 | `grafana-tcum-custom` | 每个指标分别查 labels，并按 `prometheusMetricType` 生成、执行验证 | `BuildTCUMDashboard(flavor=custom)`；query 变量配 `=~`，datasource UID 动态发现 |
| 明确 Barad 且自由指定 panel | `grafana-barad-custom` | Barad namespace/metric/label 元数据，Skill 静态检查 InfluxQL | `BuildBaradDashboard(flavor=custom)`；region 变量必有，参数必须经 `skill_exec.stdin` 传文件，避免 shell 吞单引号 |
| 客户名/UIN 的售后资源全景 | `grafana-barad-after-sales` | CRM 客户→账号→资源分布→每产品核心指标，要求维度含 appid | `BuildBaradDashboard(flavor=after_sales)`；每产品一个 Row，appid 过滤，按资源规模自适应布局 |
| 智研单指标组迁移 | `grafana-zhiyan-metric-group-transfer` | 数据源 config 优先提取凭证和 appMark，再取指标组 metadata 并试拉校验 | `BuildZhiyanDashboard`；每指标汇总/实例/全维度三联图，凭证禁止写入结果 |

共同闭环不是“生成完整 JSON”，而是：动态取得 datasource/指标/labels → LLM 规划紧凑 panel spec 和 gridPos → 查询校验 → Builder 确定性组装、marshal、上传 COS → 最终输出顶层 `GRAFANACONFIG` 代码块，块内只能有 24 小时 URL，前端据此展示一键导入。
...
Skill 源码还补充了三个关键的代码级保护：

- 编辑工具返回 `status=staged` 时并未写回 Grafana；同一会话内复用暂存副本，只有 `FlushDashboardEdits` 或会话结束自动提交才形成一个版本。
- 新增 panel 前必须证明指标来自 `FindMetrics/FindBaradMetrics`、已有 panel 或用户原文，并沿用摘要中的真实 datasource UID；不能按命名规律发明指标。
- 清空或一次删除 ≥80% panel 会被提交层拦截：自动提交直接拒绝，显式 Flush 返回 `needs_confirmation`。真正确认后才允许 `confirmDestructive=true`。
...
8. **应用大盘仍硬编码 datasource UID。** `grafana-tcum-app` 固定 `fedhvnxop3wg0c`，而 custom 路径动态发现。多环境部署时容易失效，应统一按稳定 alias 动态解析，并让 Builder 校验 UID 对应类型。
9. **参数传输方式暴露 shell 耦合。** Barad custom 已因 InfluxQL 单引号要求 `skill_exec.stdin + 文件搬运`。更好的工具契约是结构化 RPC 直接传对象，避免 SOP 教模型处理 shell quoting。
...
源码中确实注册了 CMDB 本地/MCP 工具，如服务器、PaaS 组件、业务模块、自定义模型、架构拓扑、影响面模板、异步影响面任务和影响链。现在可以从 Skill 仓证明调用路径：

- `cmdb-resource-query` 通过 `mcporter` 使用 `ListCmdbServers`、`ListCmdbServersCompact`、`ListPaasComponents`、`ListCmdbPlmModule`、`QueryCmdbModuleByPath`；大量统计必须选 compact、显式给最多 20 个 fields，截断后才把 COS 数据交给沙箱。
- `cmdb-arch-topology` 只允许拓扑/影响面六个工具和规划工具，明确禁止派发 `task` 或 `sandbox_expert`。属性盘点与关系分析因此是两条互斥路径，而不是同一条自由 ReAct 链。
- `cmdb-datasource-parse` 只解析 FlinkCDC 向导的文本/xlsx，限制 100KB/200 行或 200 条，密码只返回 `HasPassword`，禁止顺手调用写库工具。
- `cmdb-flinkcdc-script-refine` 不直接改库，而是读取前端 `msgContext.baselineSql`，优先输出 `flink_patch` 白名单 patch，由后端执行结构校验、op 白名单、SQL AST、sink 表保护和 lint；完整 `flink-sql` 只是兜底且需前端二次确认。

因此“Agent 静态 tools 为空”不等于“Skill 不知道怎么调用工具”，而是工具可通过 Skill 自带的 MCP 配置动态使用。仍待补的是运行时 Skill 版本与 MCP schema 快照，不能仅凭 Git HEAD 证明某次请求的真实轨迹。
...
1. 将“名下”映射为服务器工具的 `anyOperator`，身份最好从可信请求上下文注入；若只能从用户文本获得，应在报告中说明口径。
2. 统计型任务走 `ListCmdbServersCompact`，只请求 `zoneName` 和用于核对的最小身份字段；普通详情才走行式 `ListCmdbServers`。
3. 工具响应被截断并返回 COS URL 时才交给沙箱；否则直接对完整 compact 结果统计。
...
1. 用户未明确 mStack/model 时，`ListCustomModels` 确认 Polaris 的 `ProductSlug` 与 `ModelName`；禁止从 `ins-` 前缀猜产品。列表被截断且未找到时最多换 1—2 次关键词缩小范围，不能遍历式试错。
2. `QueryArchTopology` 以 `mstackCode`、`mstackEntityCode` 和实例 ID 查询。
3. 服务端返回节点和边，工具转换成 v3 紧凑图。
4. Agent沿 edges 总结上游依赖、下游消费者、主节点和关键属性。

单次工具最多 30 个实体；30—60 个串行分两批，明显超过 60（如 80+）应让用户缩小范围。每批关键事实需先提取到 Agent 上下文，避免后续 summarization 丢掉只存在于旧 tool message 的关系证据。
...
8. **仓库里存在能力重叠。** 新的 `cmdb-analysis` 自带 OpenAPI Python 客户端和另一套鉴权/分页流程，但不在当前四个绑定 Skill 中；应明确它是替代方案还是待合并版本，避免资源查询出现 MCP 与直连 OpenAPI 两套口径。
...
1. 加载集群总览 Skill。
2. 用户已明确“一天”时使用该窗口；未给时间时 `inspect-cluster-analysis` 默认最近 7 天，不能误写成默认一天。
...
2. 用户未明确产品时先 `ListProducts` 并停下来确认产品/版本；明确产品则直接继续。插件匹配按 Skill 要求先按 project 拉取列表，不传 keyword，再结合速查表选择并用 `GetPluginDetail` 验证。
3. `GetPluginDetail` 获取真实入参、回参和执行模式。
4. `GetResourceNodeTree` 获取真实 `root.path` 和候选 NodePaths；该生成 Skill 明确禁止调用 `ListResources`，后者只用于根因解读的 S4 资源确认阶段。
5. `ListStaticLabels` 获取可用标签。
6. 按 Skill 生成合法 JSON，且代码块语言必须严格为 `extInteraction:inspectConfig`，供前端识别并渲染配置交互。

必须如实说明写入边界：单条 `inspect-item-generate` 的职责只到 `extInteraction:inspectConfig` 配置生成；仓库中的批量 Skill 也明确禁止自己调用 `UpsertInspectItems`，只生成预览文件并由前端渲染“确认上传”按钮，用户确认后前端再调用 Upsert。也就是说，写入不是 ReAct 链里模型自由决定的一个 tool call，而是前端交互协议 + 用户确认 + 后端写接口组成的控制面闭环。
...
- 每个 Skill 自带 `config/mcporter.json`，URL、SecretId、SecretKey 通过 `skill_envs` 注入并经请求头透传；这就是 Skill 渐进加载后仍能调用远程天巡 MCP 的具体机制。
- `inspect-report-analysis` 在明细截断并返回 COS URL 时强制委派 `sandbox_expert`，且 Task 只允许 `subagent_name + description` 两个参数；未截断则直接按巡检项聚合，避免无意义启沙箱。
- 根因解读是显式四状态：S1 工具事实解读 → S2 等用户确认 → S3 沙箱源码下钻 → S4 仅当根因与资源配置有关时强制 `ListResources`。它不是一轮 ReAct 随机决定是否读源码。
...
7. **存在严重凭证治理问题。** `inspection-risk-root-cause-interpret/SKILL.md` 当前硬编码了仓库 PAT，即使文字要求不回显也已经进入 Git。应立即吊销轮换，改为 `skill_envs`/secret reference，并增加 secret scanning；文档和面试材料不得复制该值。
8. **Skill 文本有重复和格式瑕疵。** 部分文件重复声明 MCP 配置，`inspect-item-generate` 的 front matter 缩进也可疑。应在 CI 中做 YAML front matter、链接、工具 schema 和重复段落 lint。
...
Skill 源码把职责拆得比旧文更清楚：`change-order-analysis` 只是不可外显的数据采集模块，返回结构化 `ChangeContext`，不查指标、不画报告；它会对缺失 `execTime` 做兜底、筛选纳入事件并检测 overlap。上层 `change-analysis` 才加载 `dashboard-analysis`，且最终只保留均值、P95、标准差、最大值、3σ 异常点数五类统计。这个拆分避免让“取上下文”和“解释变化”互相污染。

还要注意租户失败语义：工具返回 `tenant cannot be empty` 时必须停止并提示前端选择用户空间，不能猜一个 tenant 重试；工具空结果也禁止盲目重调。
...
这四个工具在 `tcum-ai` 中没有本地实现或注册代码，说明它们来自远程 MCP/外部能力；接入表的 profile、use cases、config items 也全为空。现在虽然能读取 `tcum-tcs-system-error-analysis` Skill，但它只给出了“线索提取→错误分类→知识库多轮搜索→源码分析→报告”的 SOP，工具说明使用的是泛称 `knowledgebase_search`，并没有定义 CLS topic 发现或日志查询流程。也就是说：数据库绑定了 CLS 工具，Skill 却没有消费它们；不能把“已绑定工具”推演成“已落地日志检索链”。

## 12.2 Skill 源码可证流程

用户给出错误码、日志片段和大致时间时，Skill 的实际四步为：

1. 从 error message、错误码、堆栈、接口、服务名、requestID 和用户给出的日志片段提取线索；信息够就直接继续，只补问关键缺项。
2. 按超时、连接失败、参数/空指针、权限、限频、资源不存在、内部错误分类，并按“文件/函数→特征错误串→接口→服务+错误类型”提取搜索词。
3. 知识库先精确搜文件/函数和路由，再语义搜错误串/错误码，最后扩展到模块、配置和错误处理；找到代码后追踪抛错点、触发条件和上下游。
4. 输出问题概要、错误分析、根因、代码位置、修复建议、修改思路和预防措施；知识库覆盖不足时必须明确说明。

以上顺序是 Skill 源码可证，但没有真实 trace，不能声称某次线上请求确实执行了所有步骤。至于 `CLS_GetTopicInfoByNConverter → CLS_TextToSearchLogQuery → CLS_SearchLog`，目前只能作为待补 Workflow：除非 Agent Prompt 另有未导出的约束，否则现有 Skill 不足以保证模型会稳定走这条链。
...
6. **Skill/工具契约明显漂移。** Skill 写 `knowledgebase_search`，配置绑定名是 `knot_knowledgebase_search`；三个 CLS 工具没有进入 SOP。应在 CI 检查 Skill 引用工具是否实际绑定、绑定工具是否至少被一个分支使用，并补充只读 CLS 查询的时间/topic/limit/脱敏硬约束。
7. **“多轮搜索”没有代码预算。** Skill 鼓励多角度搜索但没有最大轮数、总时延或结果去重。应把检索预算和 query hash 去重下沉到工具/Workflow，避免 MaxStep=200 被无界检索消耗。
...
2. **Skill 可观测性不足。** Skill 仓能看到源码，主仓/数据库却只绑定名字，运行 trace 没有稳定记录实际版本和完整依赖。
...
- Grafana Skill：`/Users/yaao/Documents/code/tcum-ai-skills/skills/obs/monitor/grafana-*`。
- CMDB Skill：`/Users/yaao/Documents/code/tcum-ai-skills/skills/obs/cmdb/`。
- 天巡 Skill：`/Users/yaao/Documents/code/tcum-ai-skills/skills/obs/inspection/`。
- TCS 排障 Skill：`/Users/yaao/Documents/code/tcum-ai-skills/skills/ops/tcumtcs/tcum-tcs-system-error-analysis/SKILL.md`。
