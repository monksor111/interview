...
> 本篇不是产品功能清单，而是面试中的“场景落地证据”。内容交叉核对了两份数据库导出、`tcum-ai`、`tcum-ai-skills` 当前代码和设计 Wiki。Skill 的 SOP、脚本、MCP 配置来自 `/Users/yaao/Documents/code/tcum-ai-skills`；这能证明源码设计，但不能代替运行时发布版本和生产 trace。凡是没有生产 trace 的内容，仍明确标为“能力推演”或“待验证”。
...
| Skill 源码可证 | 能定位到 `tcum-ai-skills` 中的 `SKILL.md`、`scripts/`、`references/` 和 MCP 配置 | 可以讲路由条件、SOP、脚本调用和输出契约；是否为线上当前版本仍需发布清单证明 |
...
- Skill 从 COS 或本地后端按需加载，复杂流程先读 SOP 再调用工具。Git 仓库是 Skill 的可审计源码，不代表运行时一定加载了仓库 HEAD；严谨的发布链还应记录 `agent_config_version + skill_name + skill_version/commit + tool_schema_hash`。
...
2. Skill 固化该类任务的调用顺序、报告结构和领域规则。现在可从 `tcum-ai-skills/skills/obs/monitor` 核验完整 SOP；运行时仍通过 Skill 后端加载，因此还需发布清单确认线上版本是否与 Git HEAD 一致。
...
以“告警 ID: `ab644332cb58dd33:20260109030300`，分析告警情况”为例。这是数据库中的真实 query example，但本文没有该次执行 trace，以下流程由 Agent 配置、工具源码与 `alarm-id-analysis` Skill 联合还原：
...
sequenceDiagram
    ...
    S->>M: TimeRfcToUnixConverter + TimeRangeCalculator
    S->>P: 原始 PromQL 做瞬时查询与默认前后 30 分钟区间查询
    P-->>S: 关联指标曲线或空结果
    S-->>A: 结构化事实与证据
...
- 告警的 `status` 是判断恢复与否的唯一权威：`resolved` 为已恢复、`firing` 为未恢复。禁止根据“当前指标已回落”反推告警已恢复。
- 触发条件是异常定义，必须原样解释，不能凭常识把比较方向反转；查询时也必须保留告警原始 PromQL，不能为了拿到数据擅自改写。
- Unix 时间戳必须由时间转换工具产生。默认以告警时间为中心查询前后 30 分钟，最终展示统一为北京时间。
...
- `ListAlarmRules` 获取规则和通知配置；`AlarmHistoryByRules` 查询该策略最近 7 天历史。
- `ListAlarmRelatedRules` 查关联策略，再逐条查询关联策略历史。
- 关联强度不能靠名字相似判断：时间重叠要求同一 `timeRange` 都出现告警；对象重叠要求 `entityValue` 完全相同；两者都满足才是高相关。
- 历史触发 0 次只能说沉默，超过 50 次提示可能存在噪声，1—10 次通常较健康；这些是 Skill 的治理启发式，不是后端统计定律。
- 最终分开输出配置事实、通知配置、7 天历史、关联策略和治理建议，每条建议可回溯到工具证据。

### “分析最近一天的告警信息”

这是聚合任务。需要特别纠正：`alarm-history-analysis` 明确要求将 UTC 时间直接传给 `AlarmHistoryAnalysis`，不做额外时间转换；最终展示才换成北京时间，不能套用其他 Skill 的时间工具规则。

- `AlarmHistoryAnalysis` 获取总量、级别/状态分布、Top 策略、对象分布和时间趋势。
- 再对 Top3 策略做配置/历史下钻；所有计数必须使用工具的 `totalAlerts`、`severityStats`、`statusStats`、`topAlarmRules`、`timeRangeStats`，不能让模型对截断明细自行计数。
- 不应把“告警最多”直接等价为“根因”。最多只能说某策略或对象在这个窗口内出现最频繁；根因判断需要额外指标、日志或变更证据。

## 1.5 四条容易被忽略的 Skill 分支

- **告警配置分析**：`GenerateAlarmPromQL → AlarmPromQLAnalysis(最近 10 分钟) → ListPromQLRelatedRules → 对每条关联规则再次 AlarmPromQLAnalysis`，最终同时评价表达式语义、当前触发/噪声、级别和规则重叠。
- **基础配置**：产品、模块、实体、指标类型、指标组、指标和标签必须逐层由工具枚举；预览后可修改或分析，分析与最终确认前都必须过 `ValidateAlarmConfig`。最终只输出前端可消费的 `alarmConfig`，不直接落库。
- **高级配置**：单策略只允许一个 PromQL。多个需求必须让用户选择用 `or` 合并（共享级别和持续时间）还是拆成多条策略，不能偷偷塞进一个 Conditions 数组。
- **YunAPI/DNS 专项**：YunAPI 下钻会先用 `ListDescribeAlerts` 定位事件，再用批量 Prometheus 查询做总量、原因/错误码/客户分摊和同环比，首个必需数据为空即硬停止；DNS Skill 只负责告警事件聚合，不做 Prometheus 下钻、报告拼装或 Nebula 上报。

这里还发现一个需要治理的真实缺陷：DNS Skill 对 `alarmRuleNames` 同时写了 `域名拨测` 与 `云产品可用性-域名拨测` 两个互相冲突的固定值。它不是模型随机错误，而是 SOP 契约自身歧义，应通过 golden case + 工具 mock 在发布前拦截。
...
2. **Skill 与 Agent 主仓跨仓管理。** Skill 已有 Git 源码，但数据库绑定只存名称，看不到实际发布 commit。应给 Skill 建不可变版本，并让 Agent 配置绑定版本和工具 schema hash，才能做配置—代码原子一致性检查。
...
7. **Skill 自身也需要静态检查。** 当前已发现 DNS 固定规则名冲突、YunAPI 相近 Skill 职责重叠。发布前应检查工具名存在性、参数 schema、互斥规则、敏感信息和重复/冲突约束。
...
4. `GetDashboardIdsForInstance` 返回 `dashboardID + variables`；必须解析 variables 映射，再用 `GetDashboardConfigJson` 读取真实 panel PromQL。健康检查集合来自实例关联 Dashboard，不是 Skill 固定写死一套 CPU/内存/请求指标。
5. 替换 Grafana 变量时必须保留面板已有的所有 label 过滤条件；未被 variables 覆盖的占位符再用实例元数据补齐，并用 `ListMetricLabels` 复核。
6. 默认用时间工具生成最近 30 分钟窗口，逐条 `PrometheusQueryRange`（或按需瞬时查询），再根据真实曲线评价资源、服务、依赖和风险。
7. 输出哪些指标正常、异常或缺数据，并列明时间范围和实例信息；不直接给变更、扩容或重启建议。
...
## 2.7 Dashboard 大结果实际上怎么处理

`dashboard-analysis` 不是简单调用一次 `BatchQueryGrafana`。Skill 源码给出了完整的“规划层 + 执行层”分工：

1. 主 Agent 只读取 Dashboard/Folder 配置和 datasource map，不在这一步查数据。
2. 所有 `${var}`/`$var` 必须替换；All 变量也必须先通过查询拿到真实值，尤其 Barad 的空 `vals:[]` 容易触发 40—60 秒超时。Grafana 变量要替换，但 simpod-json 后端的 `{{start_time}}`、`{{sql_str_list(...)}}` 等模板外壳必须保留。
3. 主 Agent 产出不含未解析 Grafana 变量的 `finalQuery` 和结构化分析计划，选择通用、Grafana、Prometheus、CLS、表格、JSON 或对比模式。
4. `data-analysis` 的 `scripts/query_and_analyze.py` 通过 `mcporter call` 完成“查询→落盘→解析器→分析器→stdout JSON”；主 Agent不得绕过脚本直接查大数据，也不得把原始数据回显进上下文。
5. 报告只消费 stdout 的统计结论；`rowCount` 只用于质量检查，禁止被包装成业务指标。绝对值极小时不能只凭高百分比波动制造异常结论。

这解释了为什么工具调用列表会很长：一次 Skill 执行并不等于一次工具调用。Skill 是可复用工作流，内部可能包含变量解析、多数据源查询、脚本调度、分支重跑和结构化汇总。
...
- `cloud-platform-availability-overview`：未指定 UIN，且未指定产品或指定多个产品；平台/多产品总览。
- `cloud-product-availability-analysis`：未指定 UIN，只指定一个产品；补查产品 SLI，并下钻破线最高的 5 个 SLI 实例。
- `customer-products-availability-analysis`：指定 UIN，未指定产品或指定多个产品；从客户视角看持有产品。
- `product-customer-impact-analysis`：同时指定 UIN 和单个产品；按 `groupBys=["uin"]` 聚合，并对破线最多的 10 个客户下钻实例。
...
1. 先按“是否有 UIN × 单产品/多产品”做二维路由；本例无 UIN、多个重点产品，因此路由到平台概览 Skill。
2. `QuerySloProducts(is_vip=true)` 从重点产品缓存获取产品集合。
3. 时间转换工具生成精确毫秒时间戳。默认最近 1 小时会主动避开最后 60001ms 的数据写入延迟；长窗口按 `minute→hour→day→week→month` 自动升档，用户明确要求粒度时才不改。
...
7. 单产品 Skill 才会对破线最高 5 个 SLI 调 `QuerySliInstances`；产品影响 Skill 则取破线最高 10 个 UIN、`limit=100`。平台总览不能无条件套用这些下钻规则。
8. 需要展示客户名称时，通过云账号工具把 UIN 映射为账户信息。
9. 报告保留产品、地域原文和“破线”术语，并根据破线比例做展示色分级。

短窗口还有两种不同策略，不能混讲：平台概览、客户产品和产品客户影响将 ≤1 分钟视为快照，先定位异常对象，再只对异常对象回溯 1 小时；单产品 Skill 当前则直接把 ≤1 分钟窗口回落成最近 1 小时。这种跨 Skill 不一致应在产品层统一，否则同样的时间输入会因路由不同而改变语义。
...
数据库快照中的运行配置为 `MaxStep=100`，6 个工具、无 Skill、无子 Agent：时间转换、`FindMetrics`、`ListMetricLabels` 和 `PrometheusQuery`。但 Skill 仓库现在已经存在公开的 `tcum-promql-config`，内含查询、生成、解释、诊断四分支。严谨说法是“源码仓库已有可复用 Skill，当前数据库快照尚未证明该 Agent 已绑定它”，这属于配置与 Skill 仓的版本漂移，不能继续写成“流程只存在于 Prompt”。
...
- 必须读取 `prometheusMetricType` 决定表达式：counter 用 `rate/irate/increase`，histogram 用 `histogram_quantile + rate(_bucket)`，summary 优先查已上报 quantile，gauge 可直接聚合。语法可执行不代表指标类型语义正确。
- 生成分支用 `topk(5, <PromQL>)` 做有界验证；诊断顺序固定为“能否执行→指标类型与聚合是否匹配→label/逻辑问题”，而不是先凭经验改表达式。
...
4. **数据库绑定落后于 Skill 仓。** `tcum-promql-config` 已经把四类 Workflow Skill 化，但运行快照仍显示无 Skill；应补发布同步检查，而不是重复再设计一份。
...
数据库快照中运行配置 `MaxStep=100`，6 个工具：时间转换、namespace 列表、指标发现和 label 查询；无 Skill、无子 Agent。但 Skill 仓已有公开的 `barad-influxql-config`，同样需要区分“仓库已有”与“当前运行配置已绑定”。
...
5. 根据 `barad-influxql-config` 做静态语法与逻辑自检并解释语义；聚合字段必须使用完整的 `{metricName}:{statMethod}`，不能把 `cpu_usage:avg` 截成 `cpu_usage`。
...
2. **Skill 已存在但运行绑定不一致。** 应让数据库配置显式绑定 `barad-influxql-config`，并记录版本；Grafana Agent 与独立 InfluxQL Agent复用同一份规则。
...
- 告警：`/Users/yaao/Documents/code/tcum-ai-skills/skills/obs/monitor/alarm-*`、`yunapi-*`、`dns-probe-drill-down-analysis/`。
- 指标与大盘分析：`instance-health-diagnosis/`、`metric-query/`、`dashboard-analysis/`、`data-analysis/`；后者的 `scripts/query_and_analyze.py` 是查询与分析一体化入口。
- SLO：`cloud-platform-availability-overview/`、`cloud-product-availability-analysis/`、`customer-products-availability-analysis/`、`product-customer-impact-analysis/`。
- 查询语言：`tcum-promql-config/`、`barad-influxql-config/`。

以上路径证明当前 Git 源码设计；要证明线上某次请求，还需要对应 Agent 配置版本、Skill 发布版本和 Langfuse/运行 trace。
