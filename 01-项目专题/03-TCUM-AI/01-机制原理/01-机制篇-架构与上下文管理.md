...
**为什么要先讲坐标系**：面试一聊到 tcum-ai，很多人会开口就讲“上下文压缩、多 Agent、MCP”——但面试官往往会追问：**“一共有几个服务进程、Agent 循环到底写在哪个代码里、企业内的多个 Agent 是靠 supervisor 还是 transfer 协作？”** 如果把循环、Agent 形态和协作拓扑混在一起，就无法区分哪些能力与 eino 同层、哪些在业务代码、哪些在拼装层。后面使用“请求层、会话层、接入层”等术语时，也必须先说明这些层的位置。**坐标系不是为了背名词，而是为了让后续技术点各归其位、避免循环定义。**本节按四个坐标递进：进程职责 → 循环位置 → 5 种 Agent 形态 → 4 种协作拓扑，同时明确一个关键边界：**推理循环位于 eino ADK；名为 supervisor 的业务 Agent 不等于代码使用了 `supervisor.New`，当前主路径实际走 Deep 模式。** 这两点应主动说明。
...
**为什么上下文管理是投入最多的一块**：代码注释记录了告警诊断场景反复触发 `400 ContextWindowExceededError` 的问题：一次跨产品根因分析撞上限后，整轮工作可能直接失败。重构时发现它不是单点问题，而是一组相互关联的矛盾：工具结果可能达到 MB 级、多工具并发会让 token 非线性增长、压缩请求本身也可能超限，**任何单一手段都覆盖不了所有场景**。系统因此逐步形成七层防御：**L0 输出表示 → L1 结构化截断+COS 卸载 → L2 说明书侧压缩 → L3 周期性 summarization → L4 撞墙自适应重试 → L5 摘要替代历史 → L6 场景化裁剪**，每一层对应一类失效。反直觉的结论是：上下文管理不等于“压缩历史”，应先问“信息是否有必要进入 context”“进入后能否隔离”，最后才做有损压缩。L0～L2 主要负责从源头减少输入。
