...
**为什么会有 Skill 这一层（真实痛点现场）**：告警专家早期看到一个很困惑的现象——同样一个告警 ID，一个同学写的 prompt 能诊断到根因，一个同学写的 prompt 只能说“告警正常”，**不是模型能力差异，而是工具调用顺序不同**（前者先查指标趋势再查告警规则、后者反了过来）。又统计后发现 40+ 个行业细分场景都有各自“标准作业流程”，注入到 SystemPrompt 成本为零——但又不能全部塞进去（prompt 无限膨胀、且场景互相干扰）。不仅如此，写到代码里又会变成“每改一个流程都发版”Ｌ——运维同学想优化一句话，能不能不找一个后台同学？所以需要 Skill 回答：流程知识能不能不写代码、能不能不塞入 SystemPrompt、能不能按意图按需加载（告警场景只需告警类skill）。
