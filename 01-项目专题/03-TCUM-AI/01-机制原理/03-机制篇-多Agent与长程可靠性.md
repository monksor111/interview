...
**为什么要拆多 Agent（真实碰墙现场）**：早期想把所有能力塞进一个大 AgentＬ——告警、指标、看板、CMDB、天巡、变更，因为看上去直接、难道不是“一个 Agent 管全域”才酷？结果碰到三个官接官手的墙：

1. **system prompt 破 40k**（工具描述 25k+ / SKILL 注入 8k+ / 专家描述 4k+）Ｌ——**KV Cache 命中率降到 20% 以下**，直接花的钱翻了五倍；
...
**为什么要专门拿出来讲**：一个真实 trace 就能说明问题——`zhiyan-monitor.alarm_tag_ana ERROR → ToolNode ERROR → ReAct ERROR`Ｌ——**一个 MCP 工具报错把整个会话碰中断**，而这不是偶发事件——跨产品根因定位经常 30+ 轮工具调用、并发几十次、跨 3~4 个服务进程、现在又叠上数字分身的定时任务（cron/webhook）Ｌ**任何一环无保护，都会在长链路里放大成“整轮失败”**。而且长程任务的失败代价与短任务不在一个量级：**短任务失败重发一句就行，长任务失败 ≡ 前面 20 分钟白跑**，且用户往往不敢重发（想不想又看到 20 分钟后又报错）。所以长程任务可靠性要仔细划层、每一层都不能静默降级——本节先拆解已有的七层机制（回答"现在能挣多高"），再列十个关键缺口（回答"为什么长任务仍会碰墙"），最后给一个高 ROI 的低成本改造建议。
